...
- **Observer gamification (community)**: Observer Hub, XP and weekly quest progress, badges, and reward flows integrated with upload/home; client state in Redux (`communityGameSlice`) with local persistence and debounced sync to the auth service for verified community users (`PUT /auth/community-game`, load on sign-in). Backend validates payload bounds and stores JSON per user in SQLite (`community_game`).
