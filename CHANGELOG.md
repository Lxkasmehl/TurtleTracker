...
- **Create New Turtle E2E**: ID field now populates on WebKit/Firefox (request biology ID when sex selected; test mocks `/api/locations` and waits for generate-id). Wait for ID field value instead of generate-id response to avoid races; combobox selection fixed (ArrowDown into list, keyboard sex select in dialog); Vite strictPort so Playwright webServer readiness does not time out when port 5173 is in use.
...
- **E2E**: Sheets morphometrics test mocks upload and generate-primary-id routes, asserts POST turtle_data from response instead of route handler, and adds input value checks plus Tab after dome height for stability.
