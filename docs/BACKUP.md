...
3. **After deployment:**
   ...
   - **Cron (one job for everything):** Prefer **`scripts/daily-backup.sh`**: it runs `python -m backup.run` (Sheets, including Primary ID sync then CSV/JSON), then `backfill_folder_names.py --apply` (renames on-disk turtle folders to match the just-exported Sheets state — gender corrections, juvenile→adult bio-ID changes, primary-ID rehoming), then `backup-backend-data.sh` (images), and finally restarts the backend container **only when the backfill actually applied changes** (so the VRAM cache reloads with the new file paths). Add **one** line with **`crontab -e`**. Do **not** paste the whole line into an interactive shell—the first five fields (`0 3 * * *`) are the schedule; the shell would try to run `0` as a command and print `command not found`.
   - Use an **absolute** path for `COMPOSE_DIR` (`~` often does not expand under cron). The recommended schedule is **03:00 server time**. Example (adjust paths and log location; `bash` avoids needing the execute bit on the scripts):
     ```cron
     ...
     ```
     Ensure `BACKUP_OUTPUT_DIR` matches the host directory bind-mounted to `/app/backups` in Compose (default: `<compose dir>/backups`). If you only want Sheets (no image copy / no backfill), keep the older one-liner with `docker compose exec -T backend python -m backup.run` only.
   ...
   - **Backfill exit codes** (read by `daily-backup.sh` to decide whether to restart): `0` = no changes needed, `1` = errors (logged; backup continues), `2` = at least one rename/rehome applied (wrapper restarts the backend after the data backup). Run manually any time with `docker compose exec backend python backfill_folder_names.py` for a dry-run preview, or `--apply` to execute outside the cron window.
